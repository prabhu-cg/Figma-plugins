...
function imgBlock(name, bg) {
  ...
  const em = txt("🖼", 24, "Regular", bg||"#C8CDD6");
...
function btnNode(label, bg, textHex, pl, pt, radius) {
  ...
  f.appendChild(txt(label, 13, "SemiBold", textHex||"#FFFFFF"));
  return f;
}
...
function pillNode(label, bg, textHex) {
  const f = alFrame({name:"Badge", dir:"HORIZONTAL", main:"CENTER", cross:"CENTER",
    bg:bg, pl:10, pr:10, pt:4, pb:4, radius:99});
  f.appendChild(txt(label, 11, "Medium", textHex||"#111827"));
  return f;
}
...
function buildNav(W, bg, logoBg, linkColor, btnBg, btnText, borderColor) {
  ...
  logo.appendChild(txt("Brand", 15, "Bold", logoBg==="transparent"?"#111827":logoBg||"#111827"));
  ...
  ["Features","Pricing","About","Blog"].forEach(l => links.appendChild(txt(l,13,"Medium",linkColor||"#6B7280")));
...
const BUILDERS = {
  ...
  "nav-3": W => {
    ...
    if(W <= 480) {
      nav.appendChild(txt("ACME", 17, "Bold", "#111827"));
      const burger = alFrame({name:"Menu", dir:"VERTICAL", gap:5, pl:8, pr:8, pt:6, pb:6, main:"CENTER", cross:"CENTER"});
    ...
    ["Work","About"].forEach(l=>leftLinks.appendChild(txt(l,13,"Medium","#374151")));
    nav.appendChild(leftLinks);
    nav.appendChild(txt("ACME", 17, "Bold", "#111827"));
    const rightLinks = alFrame({name:"Right", dir:"HORIZONTAL", gap:20, pl:0,pr:0,pt:0,pb:0, cross:"CENTER"});
    ["Blog","Careers"].forEach(l=>rightLinks.appendChild(txt(l,13,"Medium","#374151")));
  ...
  "footer-1": W => {
    ...
    [["Product",["Features","Pricing","Changelog","Roadmap"]],
     ...
     ["Legal",["Privacy","Terms","Cookies","Security"]]
      ...
      col.appendChild(txt(title,11,"Bold","#9CA3AF"));
      items.forEach(i=>col.appendChild(txt(i,13,"Regular","#6B7280")));
  ...
  "footer-2": W => {
    ...
    ["Privacy","Terms","Contact","Status"].forEach(l=>links.appendChild(txt(l,13,"Regular","#6B7280")));
    f.appendChild(links);
  ...
  "footer-3": W => {
    ...
    const ph = txt("Enter your email",13,"Regular","#475569");
  ...
  "footer-4": W => {
    ...
    left.appendChild(txt("Brand",17,"Bold","#111827"));
    left.appendChild(txt("Making the internet a better place.",13,"Regular","#6B7280"));
    ...
    ["𝕏","in","gh","yt"].forEach(s=>{
      ...
      ic.appendChild(txt(s,11,"Medium","#374151"));
  ...
  "footer-5": W => {
    const f = alFrame({name:"Footer — Centred", dir:"VERTICAL", bg:"#1E1E2E", w:W,
      pl:40, pr:40, pt:40, pb:40, gap:20, cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    f.counterAxisAlignItems = "CENTER";
    f.appendChild(txt("Brand",19,"Bold","#FFFFFF",{align:"CENTER"}));
    const linkRow = alFrame({name:"Links",dir:"HORIZONTAL",gap:28,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"});
    ["Features","Pricing","About","Blog","Docs","Contact"].forEach(l=>linkRow.appendChild(txt(l,13,"Regular","#9CA3AF")));
    if(W<=480) linkRow.layoutWrap="WRAP";
    f.appendChild(linkRow);
    if(W<=480){ fill(linkRow); linkRow.primaryAxisAlignItems="CENTER"; }
    const div = rect(Math.round(W*.7),1,"#2D2D44"); f.appendChild(div);
  ...
  "footer-6": W => {
    ...
    const brandTitle=txt("Brand",17,"Bold","#FFFFFF"); fill(brandTitle); brand.appendChild(brandTitle);
    const brandSub=txt("Build better, ship faster.",13,"Regular","#6B7280"); fill(brandSub); brand.appendChild(brandSub);
  ...
  "split-1": W => {
    ...
    const sd1=txt("The platform that helps modern teams\ncollaborate and deliver.",17,"Regular","#6B7280",{lineHeight:26}); left.appendChild(sd1); fill(sd1);
  ...
  "split-2": W => {
    ...
    const sd2=txt("Built for teams that care about craft,\nconsistency and speed.",17,"Regular","#94A3B8",{lineHeight:26}); right.appendChild(sd2); fill(sd2);
  ...
  "split-4": W => {
    ...
    const sd4=txt("Photography · Film · Direction",13,"Regular","#6B7280"); right.appendChild(sd4); fill(sd4);
  ...
  "text-2": W => {
    ...
    const sd=txt("Tools and templates for designers,\ndevelopers and teams.",17,"Regular","#94A3B8",{lineHeight:26}); f.appendChild(sd); fill(sd);
  ...
  "text-3": W => {
    ...
    const sd=txt("A smarter inbox for your team.",17,"Regular","#6B7280",{align:"CENTER"}); f.appendChild(sd); fill(sd);
  ...
  "stats-1": W => {
    ...
    const buildCard=([v,l],parent)=>{
      const card=alFrame({name:l,dir:"VERTICAL",bg:"#F9FAFB",gap:8,pl:20,pr:20,pt:20,pb:20,radius:10}); parent.appendChild(card); fill(card); fillV(card);
      card.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; card.strokeWeight=1;
      const vt=txt(v,26,"Bold","#111827"); card.appendChild(vt); fill(vt);
      const lt=txt(l,13,"Regular","#6B7280"); card.appendChild(lt); fill(lt);
    };
  ...
  "stats-2": W => {
    ...
    const buildStat=([v,l],parent)=>{
      ...
      const lt=txt(l,13,"Regular","#6B7280"); s.appendChild(lt); fill(lt);
    };
    const makeRow=(parent)=>{
  ...
  "stats-3": W => {
    ...
    const buildStat=([v,l],parent)=>{
      ...
      const lt=txt(l,13,"Regular","#6B7280",{align:"CENTER"}); s.appendChild(lt); fill(lt);
    };
    const makeRow=(parent)=>{
      const row=alFrame({name:"Stats",dir:"HORIZONTAL",gap:40,pl:0,pr:0,pt:0,pb:0}); parent.appendChild(row); row.resize(1,80); row.counterAxisSizingMode="FIXED"; fill(row);
      return row;
  ...
  "stats-4": W => {
    ...
    const buildStat=([v,l],parent)=>{
      ...
      const lt=txt(l,13,"Regular","#94A3B8"); s.appendChild(lt); fill(lt);
    };
    const makeRow=(parent)=>{
      const row=alFrame({name:"Stats",dir:"HORIZONTAL",gap:40,pl:0,pr:0,pt:0,pb:0}); parent.appendChild(row); row.resize(1,80); row.counterAxisSizingMode="FIXED"; fill(row);
      return row;
  ...
  "stats-5": W => {
    ...
    const buildStat=(d,parent)=>{
      ...
      const lt=txt(d.l,13,"Regular","#64748B",{align:"CENTER"}); s.appendChild(lt); fill(lt);
    };
    const makeRow=(parent)=>{
  ...
  "stats-6": W => {
    ...
    const buildCard=([v,l,bg,tc],parent)=>{
      const card=alFrame({name:l,dir:"VERTICAL",bg,gap:8,pl:20,pr:20,pt:20,pb:20,radius:10}); parent.appendChild(card); fill(card); fillV(card);
  ...
  "feat-1": W => {
    ...
    [{icon:"⚡",t:"Lightning Fast",d:"Zero-compromise performance."},{icon:"🔒",t:"Secure by default",d:"Enterprise-grade security built in."},{icon:"🔗",t:"200+ Integrations",d:"Works with tools you already use."}].forEach(feat=>{
      ...
      const tt=txt(feat.t,15,"SemiBold","#111827"); card.appendChild(tt); fill(tt);
      const d=txt(feat.d,13,"Regular","#6B7280",{lineHeight:20}); card.appendChild(d); fill(d);
    });
    return f;
  },
  "feat-2": W => {
    ...
    feats.forEach((feat,i)=>{
      ...
      ic.appendChild(txt(feat.icon,15,"Regular","#7C3AED")); item.appendChild(ic);
      const col=alFrame({name:"col",dir:"VERTICAL",gap:3,pl:0,pr:0,pt:0,pb:0}); item.appendChild(col); fill(col);
      const tt=txt(feat.t,13,"SemiBold","#111827"); col.appendChild(tt); fill(tt);
  ...
  "feat-3": W => {
    ...
    [{icon:"⚡",t:"Lightning Fast",d:"Zero-compromise performance."},{icon:"🔒",t:"Secure by default",d:"Enterprise-grade security."},{icon:"🔗",t:"Integrations",d:"200+ tools you already use."},{icon:"📊",t:"Analytics",d:"Real-time actionable insights."}].forEach(feat=>{
      ...
      const ck=alFrame({name:"ck",dir:"HORIZONTAL",bg:"#D1FAE5",pl:4,pr:4,pt:4,pb:4,radius:99,main:"CENTER",cross:"CENTER"});
      ck.appendChild(txt("✓",11,"Bold","#059669")); row.appendChild(ck);
      ...
      const dt=txt(feat.d,13,"Regular","#6B7280"); col.appendChild(dt); fill(dt);
    });
  ...
  "feat-4": W => {
    ...
    [{icon:"⚡",t:"Lightning Fast",d:"Zero-compromise performance."},{icon:"🔒",t:"Secure by default",d:"Enterprise-grade security."},{icon:"🔗",t:"200+ Integrations",d:"Works with tools you already use."}].forEach(feat=>{
      ...
      const tt=txt(feat.t,15,"SemiBold","#F1F5F9"); card.appendChild(tt); fill(tt);
      const d=txt(feat.d,13,"Regular","#64748B",{lineHeight:20}); card.appendChild(d); fill(d);
    });
    return f;
  },
  "feat-5": W => {
    ...
    const buildStep=(step,idx,parent)=>{
      ...
      const tt=txt(step.t,15,"SemiBold","#F1F5F9"); col.appendChild(tt); fill(tt);
      const d=txt(step.d,13,"Regular","#64748B",{lineHeight:20}); col.appendChild(d); fill(d);
    };
    if(mobile){
  ...
  "feat-6": W => {
    ...
    [{icon:"⚡",t:"Lightning Fast",d:"Zero-compromise performance on every device.",side:"right"},{icon:"🔒",t:"Secure by default",d:"Enterprise-grade security built in from day one.",side:"left"},{icon:"🔗",t:"200+ Integrations",d:"Connect tools your team already loves and uses.",side:"right"}].forEach(feat=>{
      ...
      const buildCol=()=>{
        ...
        const ic=alFrame({name:"ic",dir:"HORIZONTAL",bg:"#EDE9FE",pl:10,pr:10,pt:10,pb:10,radius:10,main:"CENTER",cross:"CENTER"});
        ic.appendChild(txt(feat.icon,22,"Regular","#7C3AED")); col.appendChild(ic);
        const tt=txt(feat.t,17,"Bold","#111827"); col.appendChild(tt); fill(tt);
  ...
  "cta-3": W => {
    ...
    const sd=txt("Get weekly tips, templates and tools delivered to your inbox.",15,"Regular","#6B7280",{align:"CENTER"}); f.appendChild(sd); fill(sd);
    ...
    const ph=txt("Enter your email address…",13,"Regular","#9CA3AF"); ph.resize(280,20); input.appendChild(ph);
  ...
  "cta-4": W => {
    ...
    const sd=txt("Join 10,000+ teams and ship faster.",15,"Regular","#78350F"); left.appendChild(sd); fill(sd);
  ...
  "cta-6": W => {
    ...
    const sd=txt("Start free — no credit card needed. Upgrade anytime.",15,"Regular","#A5B4FC",{align:"CENTER"}); f.appendChild(sd); fill(sd);
  ...
  "test-1": W => {
    ...
    [{q:"This product completely changed how we ship.",who:"Alex J",role:"CTO, Acme"},
     ...
     {q:"Incredibly intuitive and powerful.",who:"James L",role:"Lead Eng, Arc"}].forEach(q=>{
      ...
      const qt=txt('"'+q.q+'"',13,"Regular","#374151",{lineHeight:20}); card.appendChild(qt); fill(qt);
      const wt=txt(q.who,13,"Bold","#111827"); card.appendChild(wt); fill(wt);
  ...
  "test-3": W => {
    ...
    [{q:"This product changed how we ship.",who:"Alex J",role:"CTO, Acme"},
     ...
     {q:"Incredibly intuitive and powerful.",who:"James L",role:"Lead Eng, Arc"}].forEach(q=>{
      ...
      const st=txt("★★★★★",14,"Regular","#F59E0B"); card.appendChild(st); fill(st);
      const qt=txt('"'+q.q+'"',13,"Regular","#D1D5DB",{lineHeight:20}); card.appendChild(qt); fill(qt);
      const wt=txt(q.who,13,"Bold","#F9FAFB"); card.appendChild(wt); fill(wt);
      const rt=txt(q.role,12,"Regular","#6B7280"); card.appendChild(rt); fill(rt);
    });
    return f;
  },
  "test-4": W => {
    ...
    [{who:"Alex J",role:"CTO, Acme",q:"Changed how we ship."},
     ...
     {who:"Priya P",role:"PM, Loop",q:"Velocity tripled."}].forEach(q=>{
      ...
      const av=alFrame({name:"av",dir:"HORIZONTAL",bg:"#E5E7EB",pl:0,pr:0,pt:0,pb:0,radius:99,main:"CENTER",cross:"CENTER"});
      ...
      const wt=txt(q.who,13,"Bold","#111827"); col.appendChild(wt); fill(wt);
      const rt=txt(q.role,11,"Regular","#6B7280"); col.appendChild(rt); fill(rt);
      const qt=txt('"'+q.q+'"',13,"Regular","#374151",{lineHeight:20}); col.appendChild(qt); fill(qt);
    });
    return f;
  },
  "test-5": W => {
    ...
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const tt=txt("Trusted by teams at",13,"Regular","#9CA3AF",{align:"CENTER"}); f.appendChild(tt); fill(tt);
    ...
    ["Acme","Nova","Loop","Arc","Prism","Delta"].forEach(l=>{
      ...
      const lt=txt(l,13,"Bold","#9CA3AF"); b.appendChild(lt); fill(lt);
    });
    return f;
  },
  "test-6": W => {
  ...
  "team-1": W => {
    ...
    const eyebrow=txt("OUR TEAM",11,"SemiBold","#EE661D"); eyebrow.name="Eyebrow"; eyebrow.textAlignHorizontal="CENTER"; hdr.appendChild(eyebrow); fill(eyebrow);
    ...
    const sub=txt("A talented group of creators, thinkers, and builders.",15,"Regular","#6B7280",{align:"CENTER"}); sub.name="Subheading"; hdr.appendChild(sub); fill(sub);
    ...
    [["Alex Rivera","CEO & Founder"],["Jordan Lee","Head of Design"],["Sam Chen","Lead Engineer"],["Casey Park","Product Manager"]].forEach(([n,r])=>{
      ...
      card.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; card.strokeWeight=1;
      const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#E5E7EB",pl:0,pr:0,pt:0,pb:0,radius:99,main:"CENTER",cross:"CENTER"});
  ...
  "team-2": W => {
    ...
    const st=txt("The people building the future with us.",15,"Regular","#6B7280"); hdr.appendChild(st); fill(st);
    const row=alFrame({name:"Members",dir:mobile?"VERTICAL":"HORIZONTAL",gap:40,pl:0,pr:0,pt:0,pb:0}); f.appendChild(row); fill(row);
    [["Alex R.","Engineering"],["Jordan L.","Design"],["Sam C.","Product"],["Casey P.","Marketing"]].forEach(([n,dept])=>{
      const card=alFrame({name:"Member — "+n,dir:"VERTICAL",bg:"#1F2937",gap:10,pl:20,pr:20,pt:20,pb:20,radius:10}); row.appendChild(card); fill(card);
      const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#374151",pl:0,pr:0,pt:0,pb:0,radius:99,main:"CENTER",cross:"CENTER"});
  ...
  "team-4": W => {
    ...
    [["Alex Rivera","CEO & Founder","Building at the intersection of design and engineering."],
     ...
     ["Casey Park","Product Manager","User research and roadmapping are my superpowers."]].forEach(([n,r,bio])=>{
      ...
      const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#F3F4F6",pl:0,pr:0,pt:0,pb:0,radius:99,main:"CENTER",cross:"CENTER"});
      ...
      const bt=txt(bio,13,"Regular","#9CA3AF",{lineHeight:20}); bt.name="Bio"; info.appendChild(bt); fill(bt);
    });
  ...
  "team-5": W => {
    ...
    const sh=txt("Meet the team behind the product.",15,"Regular","#6B7280"); hdr.appendChild(sh); fill(sh);
    const row=alFrame({name:"Members",dir:mobile?"VERTICAL":"HORIZONTAL",gap:40,pl:0,pr:0,pt:0,pb:0}); f.appendChild(row); fill(row);
    [["Alex Rivera","CEO","Believer in great design. 10+ years building products people love.","#7C3AED"],
     ...
     ["Casey Park","Product","Obsessed with understanding users and building what they need.","#F59E0B"]].forEach(([n,r,bio,ac])=>{
      ...
      const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:ac,pl:0,pr:0,pt:0,pb:0,radius:99,main:"CENTER",cross:"CENTER"});
      av.resize(44,44); av.primaryAxisSizingMode="FIXED"; av.counterAxisSizingMode="FIXED";
      ...
      const bt=txt(bio,13,"Regular","#6B7280",{lineHeight:20}); bt.name="Bio"; card.appendChild(bt); fill(bt);
    });
    return f;
  },
  "team-6": W => {
    ...
    const ld=txt("Small team. Big ambitions. We move fast and care deeply about the work.",15,"Regular","#6B7280",{lineHeight:24}); left.appendChild(ld); fill(ld);
    ...
    [["Alex Rivera","CEO & Founder"],["Jordan Lee","Head of Design"],["Sam Chen","Lead Engineer"],["Casey Park","Product Manager"],["Morgan Wu","Head of Growth"]].forEach(([n,r])=>{
      ...
      av.appendChild(txt(n[0],13,"Bold","#9CA3AF",{align:"CENTER"})); item.appendChild(av);
      const info=alFrame({name:"Info",dir:"VERTICAL",gap:2,pl:0,pr:0,pt:0,pb:0}); item.appendChild(info); fill(info);
      const nt=txt(n,13,"SemiBold","#F9FAFB"); info.appendChild(nt); fill(nt);
      const rt=txt(r,11,"Regular","#6B7280"); info.appendChild(rt); fill(rt);
    });
    return f;
  },
  ...
  "faq-1": W => {
    ...
    const ey=txt("FAQ",11,"SemiBold","#EE661D"); ey.name="Eyebrow"; ey.textAlignHorizontal="CENTER"; hdr.appendChild(ey); fill(ey);
    ...
    [["How do I get started?","Sign up for free and you'll be designing in minutes. No credit card required.",true],
     ...
     ["Do you offer refunds?","We offer a 30-day money-back guarantee, no questions asked.",false]
      ...
      const qt=txt(q,15,"SemiBold","#111827"); qRow.appendChild(qt); fill(qt);
    ...
    [[["How do I get started?","Sign up for free in minutes. No credit card required."],["Can I collaborate?","Invite unlimited team members and work together in real-time."]],
     [["What about billing?","Monthly and annual plans. Cancel anytime."],["Is my data secure?","Enterprise-grade encryption and SOC2 certified."]]
      ...
      qs.forEach(([q,a])=>{
        ...
        const qt=txt(q,15,"SemiBold","#111827"); item.appendChild(qt); fill(qt);
  ...
  "faq-3": W => {
    ...
    [["What is Pattern Pilot?","A Figma plugin for generating production-ready section layouts instantly."],
     ...
     ["Can I customise the sections?","Absolutely. Every layer is named and structured for easy editing."]
      ...
      const qt=txt(q,15,"SemiBold","#F1F5F9"); item.appendChild(qt); fill(qt);
  ...
  "faq-4": W => {
    ...
    [["01","Is there a free plan?","Yes — our free tier includes 3 projects with no time limit."],
     ...
     ["04","Do you offer team plans?","Team plans start from 3 seats with centralised billing."]
      ...
      const nb=alFrame({name:"Number Badge",dir:"HORIZONTAL",bg:"#FFF9EB",pl:10,pr:10,pt:6,pb:6,radius:6,main:"CENTER",cross:"CENTER"});
      nb.appendChild(txt(num,12,"Bold","#EE661D")); row.appendChild(nb);
      const col=alFrame({name:"Content",dir:"VERTICAL",gap:6,pl:0,pr:0,pt:0,pb:0}); row.appendChild(col); fill(col);
      const qt=txt(q,15,"SemiBold","#111827"); col.appendChild(qt); fill(qt);
      const at=txt(a,13,"Regular","#6B7280",{lineHeight:20}); col.appendChild(at); fill(at);
    });
    return f;
  },
  "faq-5": W => {
    ...
    [["What payment methods do you accept?","All major credit cards, PayPal, and bank transfers for annual plans."],
     ...
     ["How do I cancel?","Cancel anytime from Settings → Billing. No penalties or fees."]
      ...
      const at=txt(a,13,"Regular","#6B7280",{lineHeight:20}); item.appendChild(at); fill(at);
    });
    return f;
  },
  "faq-6": W => {
    ...
    const sd=txt("Everything you need to know about the product.",15,"Regular","#6B7280",{align:"CENTER"}); f.appendChild(sd); fill(sd);
    ...
    [["🔓","Getting Started","Sign up free, pick a plan, invite your team. Live in minutes."],
     ...
     ["💬","Support","24/7 live chat. Average response under 2 hours."]
      ...
      const at=txt(a,13,"Regular","#6B7280",{lineHeight:20}); card.appendChild(at); fill(at);
    });
    return f;
  },
  ...
  "news-1": W => {
    ...
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    f.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; f.strokeWeight=1; f.strokeAlign="OUTSIDE";
    const ey=txt("STAY UPDATED",11,"SemiBold","#EE661D"); ey.name="Eyebrow"; ey.textAlignHorizontal="CENTER"; f.appendChild(ey); fill(ey);
    const hd=txt("Get the Latest News",30,"Bold","#111827",{align:"CENTER"}); hd.name="Heading"; f.appendChild(hd); fill(hd);
    const sd=txt("Subscribe and stay up-to-date with the latest features, tips, and insights.",15,"Regular","#6B7280",{align:"CENTER"}); sd.name="Subheading"; f.appendChild(sd); fill(sd);
    f.appendChild(btnNode("Subscribe","#EE661D","#FFFFFF",28,13,8));
  ...
  "news-2": W => {
    ...
    const btn=btnNode("Subscribe","#EE661D","#FFFFFF",20,12,0); row.appendChild(btn); if(mobile) fill(btn);
    return f;
  },
  "news-3": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Newsletter — Inline Strip",dir:mobile?"VERTICAL":"HORIZONTAL",bg:"#EE661D",w:W,pl:40,pr:40,pt:40,pb:40,gap:40,cross:"CENTER"});
    if(mobile){ f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED"; } else { f.counterAxisSizingMode="AUTO"; }
    ...
    const st=txt("Weekly digest. No spam, ever.",13,"Regular","#FEE2E2"); left.appendChild(st); fill(st);
    ...
    const ph=txt("your@email.com",13,"Regular","#9CA3AF"); ph.name="Placeholder"; input.appendChild(ph); fill(ph);
  ...
  "news-4": W => {
    ...
    input.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; input.strokeWeight=1;
    const ph=txt("Enter your email",13,"Regular","#9CA3AF"); ph.name="Placeholder"; input.appendChild(ph); fill(ph);
  ...
  "news-5": W => {
    ...
    const sd=txt("Weekly newsletter trusted by 12,000+ designers and builders.",15,"Regular","#94A3B8",{lineHeight:24}); left.appendChild(sd); fill(sd);
    ...
    const btn=btnNode("Get the newsletter →","#EE661D","#FFFFFF",20,12,8); right.appendChild(btn); fill(btn);
    const fn=txt("No spam. Unsubscribe anytime.",11,"Regular","#4B5563"); right.appendChild(fn); fill(fn);
    return f;
  },
  "news-6": W => {
    const mobile=W<=480;
    ...
    const st=txt("Monthly product updates and design inspiration.",13,"Regular","#6B7280"); left.appendChild(st); fill(st);
    const row=alFrame({name:"Subscribe Row",dir:"HORIZONTAL",gap:0,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(row); fill(row);
    ...
    input.strokes=[{type:"SOLID",color:rgb("#D1D5DB")}]; input.strokeWeight=1;
    const ph=txt("you@email.com",13,"Regular","#9CA3AF"); ph.name="Placeholder"; input.appendChild(ph); fill(ph);
  ...
  "price-1": W => {
    ...
    const sd=txt("No hidden fees. Switch plans anytime.",15,"Regular","#6B7280",{align:"CENTER"}); f.appendChild(sd); fill(sd);
    ...
    [{plan:"Free",price:"$0",period:"/month",desc:"For individuals getting started.",features:["3 projects","Basic analytics","Community support"],btn:"Get Started",btnBg:"none",btnTxt:"#374151"},
     {plan:"Pro",price:"$29",period:"/month",desc:"For growing teams and businesses.",features:["Unlimited projects","Advanced analytics","Priority support","Custom domains"],btn:"Start Free Trial",btnBg:"#EE661D",btnTxt:"#FFFFFF",hi:true},
     {plan:"Enterprise",price:"Custom",period:"",desc:"For large organisations.",features:["Everything in Pro","SSO & SAML","SLA guarantee","Dedicated support"],btn:"Contact Sales",btnBg:"#111827",btnTxt:"#FFFFFF"}
      ...
      if(hi){card.strokes=[{type:"SOLID",color:rgb("#EE661D")}]; card.strokeWeight=2;}
      else{card.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; card.strokeWeight=1;}
      if(hi) card.appendChild(pillNode("Popular","#FFF9EB","#EE661D"));
      card.appendChild(txt(plan,13,"SemiBold","#6B7280"));
      ...
      const dt=txt(desc,13,"Regular","#6B7280",{lineHeight:20}); card.appendChild(dt); fill(dt);
      ...
      features.forEach(f2=>{
        ...
        const ft=txt(f2,13,"Regular","#374151"); fr.appendChild(ft); fill(ft);
      });
  ...
  "price-2": W => {
    ...
    [{plan:"Starter",price:"$0",features:["3 projects","5GB storage","Email support"]},
     ...
     {plan:"Scale",price:"$49",features:["Unlimited","100GB storage","24/7 support","Custom integrations"]}
      ...
      card.strokes=[{type:"SOLID",color:rgb(hi?"#EE661D":"#1E293B")}]; card.strokeWeight=hi?2:1;
      ...
      features.forEach(f2=>{
        const fr=alFrame({name:"Feature",dir:"HORIZONTAL",gap:8,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); card.appendChild(fr); fill(fr);
        fr.appendChild(txt("→",12,"Regular","#EE661D"));
        const ft=txt(f2,13,"Regular","#94A3B8"); fr.appendChild(ft); fill(ft);
      });
      const bb=btnNode(hi?"Get started":"Choose",hi?"#EE661D":"none",hi?"#FFFFFF":"#94A3B8",16,10,8); card.appendChild(bb); fill(bb);
    });
    return f;
  },
  "price-3": W => {
    ...
    [{plan:"Basic",price:"$9",desc:"For freelancers",bg:"#F9FAFB",border:"#E5E7EB",tc:"#111827"},
     {plan:"Pro",price:"$29",desc:"Best for growing teams",bg:"#EE661D",border:"#EE661D",tc:"#FFFFFF",hi:true},
     {plan:"Business",price:"$79",desc:"For enterprise teams",bg:"#F9FAFB",border:"#E5E7EB",tc:"#111827"}
      ...
      card.appendChild(txt(desc,13,"Regular",hi?"#FEE2E2":"#6B7280"));
      const bb=btnNode(hi?"Start Now":"Get started",hi?"#FFFFFF":"none",hi?"#EE661D":tc,16,10,6); card.appendChild(bb); fill(bb);
    });
    return f;
  },
  "price-4": W => {
    ...
    const toggle=alFrame({name:"Billing Toggle",dir:"HORIZONTAL",bg:"#E5E7EB",gap:0,pl:4,pr:4,pt:4,pb:4,radius:99,cross:"CENTER"});
    f.appendChild(toggle);
    const monthly=alFrame({name:"Monthly (Active)",dir:"HORIZONTAL",bg:"#FFFFFF",pl:16,pr:16,pt:6,pb:6,radius:99,main:"CENTER",cross:"CENTER"});
    monthly.appendChild(txt("Monthly",12,"SemiBold","#111827")); toggle.appendChild(monthly);
    const annual=alFrame({name:"Annual",dir:"HORIZONTAL",pl:16,pr:16,pt:6,pb:6,radius:99,main:"CENTER",cross:"CENTER"});
    ...
    [{plan:"Solo",price:"$12",features:["1 seat","10 projects","Email support"]},
     ...
     {plan:"Agency",price:"$79",features:["20 seats","Unlimited everything","Dedicated support","White label"]}
      ...
      pr.appendChild(txt(price,32,"Bold","#111827")); pr.appendChild(txt("/mo",13,"Regular","#9CA3AF"));
      features.forEach(f2=>{
        const fr=alFrame({name:"Feature",dir:"HORIZONTAL",gap:8,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); card.appendChild(fr); fill(fr);
        fr.appendChild(txt("✓",12,"Bold",hi?"#EE661D":"#16A34A"));
        const ft=txt(f2,13,"Regular","#374151"); fr.appendChild(ft); fill(ft);
      });
      const bb=btnNode(hi?"Start free trial":"Choose "+plan,hi?"#EE661D":"none",hi?"#FFFFFF":"#374151",14,9,8); card.appendChild(bb); fill(bb);
    });
    return f;
  },
  "price-5": W => {
    ...
    const sd=txt("Free forever. Upgrade when you're ready.",15,"Regular","#6B7280",{align:"CENTER"}); f.appendChild(sd); fill(sd);
    const row=alFrame({name:"Plans",dir:mobile?"VERTICAL":"HORIZONTAL",gap:24,pl:0,pr:0,pt:0,pb:0,cross:"MIN"}); f.appendChild(row); fill(row);
    [{plan:"Free",price:"$0",desc:"Start building for free.",features:["3 active projects","1GB storage","Community support","Basic analytics"],btn:"Start Free",btnBg:"#F3F4F6",btnTxt:"#111827"},
     {plan:"Pro",price:"$25",desc:"Everything you need to grow.",features:["Unlimited projects","50GB storage","Priority support","Advanced analytics","Custom domain","Team collaboration"],btn:"Start 14-day Trial",btnBg:"#EE661D",btnTxt:"#FFFFFF",hi:true}
      ...
      const dt=txt(desc,13,"Regular","#6B7280"); card.appendChild(dt); fill(dt);
      ...
      features.forEach(f2=>{
        const fr=alFrame({name:"Feature",dir:"HORIZONTAL",gap:8,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); card.appendChild(fr); fill(fr);
        fr.appendChild(txt(hi?"✦":"✓",11,"SemiBold",hi?"#EE661D":"#16A34A"));
        const ft=txt(f2,13,"Regular","#374151"); fr.appendChild(ft); fill(ft);
      });
  ...
  "price-6": W => {
    ...
    if(mobile){
      ...
      features.forEach((feat,i)=>{
        ...
        const ft=txt(feat,13,"SemiBold","#111827"); card.appendChild(ft); fill(ft);
      ...
      ["US","OTHERS"].forEach(label=>{
        ...
        col.appendChild(txt(label,11,"Bold",label==="US"?"#EE661D":"#9CA3AF"));
      });
      features.forEach((feat,i)=>{
        ...
        const ft=txt(feat,13,"Regular","#374151"); fc.appendChild(ft); fill(ft);
  ...
  "bento-1": W => {
    ...
    ctaRow.appendChild(btnNode("Get Started","#EE661D","#FFFFFF",20,11,8));
    ...
    [{name:"Analytics",icon:"📊",desc:"Real-time insights",bg:"#1E293B"},{name:"Collaboration",icon:"🤝",desc:"Work together",bg:"#1E3A5F"},{name:"Security",icon:"🔒",desc:"Enterprise grade",bg:"#1E293B"}].forEach(({name,icon,desc,bg})=>{
  ...
  "bento-3": W => {
    ...
    const left=alFrame({name:"Main Block",dir:"VERTICAL",bg:"#EE661D",gap:20,pl:32,pr:32,pt:40,pb:40,radius:16});
    f.appendChild(left);
    ...
    left.appendChild(btnNode("Explore","#FFFFFF","#EE661D",20,10,8));
  ...
  "bento-4": W => {
    ...
    if(mobile){
      [{name:"Performance",bg:"#1E293B",desc:"10x faster builds"},{name:"Global CDN",bg:"#0EA5E9",desc:"Edge delivery in 40+ regions"},{name:"Security",bg:"#312E81",desc:""},{name:"Uptime",bg:"#1E293B",desc:""},{name:"Support",bg:"#1E3A5F",desc:""}].forEach(({name,desc,bg})=>{
        const card=alFrame({name:"Tile — "+name,dir:"VERTICAL",bg,gap:8,pl:20,pr:20,pt:20,pb:20,radius:12}); f.appendChild(card); fill(card);
        const nt=txt(name,15,"Bold","#FFFFFF"); card.appendChild(nt); fill(nt);
        if(desc){const dt=txt(desc,13,"Regular","#94A3B8"); card.appendChild(dt); fill(dt);}
      });
    } else {
      ...
      [{name:"Performance",bg:"#1E293B",desc:"10x faster builds"},{name:"Global CDN",bg:"#0EA5E9",desc:"Edge delivery in 40+ regions"}].forEach(({name,desc,bg})=>{
        ...
        const dt=txt(desc,13,"Regular","#94A3B8"); card.appendChild(dt); fill(dt);
      });
      const row2=alFrame({name:"Row 2",dir:"HORIZONTAL",gap:40,pl:0,pr:0,pt:0,pb:0,cross:"MIN"}); f.appendChild(row2); fill(row2);
      [{name:"Security",bg:"#312E81"},{name:"Uptime",bg:"#1E293B"},{name:"Support",bg:"#1E3A5F"}].forEach(({name,bg})=>{
  ...
  "bento-5": W => {
    ...
    cta.appendChild(btnNode("Get started →","#EE661D","#FFFFFF",20,11,8));
    ...
    [{name:"Design Tools",bg:"#111827"},{name:"Dev Handoff",bg:"#F0FDF4",border:"#BBF7D0"},{name:"Team Collab",bg:"#EFF6FF",border:"#BFDBFE"}].forEach(({name,bg,border})=>{
      ...
      fill(card);
      const nt=txt(name,15,"Bold",bg==="#111827"?"#FFFFFF":"#111827"); card.appendChild(nt); fill(nt);
    });
    return f;
  },
  "bento-6": W => {
    ...
    const lr=txt("Repeat.",mobile?28:40,"Bold","#EE661D",{lineHeight:mobile?36:48}); left.appendChild(lr); fill(lr);
    const ldesc=txt("The complete toolkit for teams that ship.",15,"Regular","#6B7280"); left.appendChild(ldesc); fill(ldesc);
    left.appendChild(btnNode("Start for free","#EE661D","#FFFFFF",22,12,8));
    ...
    [{name:"Fast Builds",bg:"#FFF9EB",border:"#FDE68A"},{name:"Global Scale",bg:"#EDE9FE",border:"#DDD6FE"}].forEach(({name,bg,border})=>{
      const c=alFrame({name:"Tile — "+name,dir:"VERTICAL",bg,gap:4,pl:16,pr:16,pt:16,pb:16,radius:10}); topRow.appendChild(c);
    ...
    const mid=alFrame({name:"Tile — Analytics",dir:"HORIZONTAL",bg:"#111827",gap:16,pl:20,pr:20,pt:16,pb:16,radius:10,cross:"CENTER"}); cluster.appendChild(mid); fill(mid);
    ...
    [{name:"Security",bg:"#ECFDF5",border:"#A7F3D0"},{name:"Support",bg:"#F0F9FF",border:"#BAE6FD"},{name:"API",bg:"#FFF1F2",border:"#FECDD3"}].forEach(({name,bg,border})=>{
      const c=alFrame({name:"Tile — "+name,dir:"VERTICAL",bg,gap:4,pl:12,pr:12,pt:12,pb:12,radius:10}); botRow.appendChild(c);
      if(border){c.strokes=[{type:"SOLID",color:rgb(border)}]; c.strokeWeight=1;}
      c.resize(1,80); c.primaryAxisSizingMode="FIXED"; fill(c);
      const cn=txt(name,13,"Bold","#111827"); c.appendChild(cn); fill(cn);
    });
    return f;
  },
  ...
  "gallery-1": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Hero — Caption Row",dir:"VERTICAL",bg:"#FFFFFF",w:W,pt:60,pb:60,pl:40,pr:40,gap:40,cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const hdr=alFrame({name:"Header",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(hdr); fill(hdr);
    ...
    hdr.appendChild(btnNode("Start Creating","#EE661D","#FFFFFF",22,12,8));
    ...
    [{name:"Pixel Perfect",img:"#F97316",desc:"Every detail crafted to look sharp on any screen."},{name:"Lightning Fast",img:"#0EA5E9",desc:"Go from idea to finished design in minutes."},{name:"Fully Flexible",img:"#64748B",desc:"Customize everything to match your brand and style."}].forEach(({name,img,desc})=>{
      const card=alFrame({name:"Card — "+name,dir:"VERTICAL",gap:12,pl:0,pr:0,pt:0,pb:0}); grid.appendChild(card); fill(card);
      const imgF=alFrame({name:"Image",dir:"VERTICAL",bg:img,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); card.appendChild(imgF); fill(imgF);
      imgF.resize(1,mobile?160:220); imgF.primaryAxisSizingMode="FIXED";
      const cap=alFrame({name:"Caption",dir:"VERTICAL",gap:4,pl:0,pr:0,pt:0,pb:0}); card.appendChild(cap); fill(cap);
      const cn=txt(name,15,"Bold","#111827"); cap.appendChild(cn); fill(cn);
      const cd=txt(desc,13,"Regular","#6B7280",{lineHeight:20}); cap.appendChild(cd); fill(cd);
    });
    return f;
  },
  "gallery-2": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Hero — Dark Showcase",dir:"VERTICAL",bg:"#0F172A",w:W,pt:60,pb:60,pl:40,pr:40,gap:40,cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const hdr=alFrame({name:"Header",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(hdr); fill(hdr);
    ...
    hdr.appendChild(btnNode("Explore Work","#EE661D","#FFFFFF",22,12,8));
    const grid=alFrame({name:"Gallery Grid",dir:mobile?"VERTICAL":"HORIZONTAL",gap:24,pl:0,pr:0,pt:0,pb:0,cross:"MIN"}); f.appendChild(grid); fill(grid);
    [{name:"Brand Identity",img:"#8B5CF6"},{name:"Motion Design",img:"#0EA5E9"},{name:"Product UX",img:"#10B981"}].forEach(({name,img})=>{
      ...
      const imgF=alFrame({name:"Image",dir:"VERTICAL",bg:img,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); card.appendChild(imgF); fill(imgF);
      imgF.resize(1,mobile?160:200); imgF.primaryAxisSizingMode="FIXED";
  ...
  "gallery-3": W => {
    ...
    const tablet=!mobile&&W<=768;
    const stacked=mobile||tablet;
    const f=alFrame({name:"Hero — Editorial Grid",dir:stacked?"VERTICAL":"HORIZONTAL",bg:"#FFFFFF",w:W,pt:40,pb:40,pl:40,pr:40,gap:40,cross:stacked?"MIN":"CENTER"});
    if(stacked){f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";}
    const left=alFrame({name:"Copy",dir:"VERTICAL",gap:20,pl:0,pr:0,pt:0,pb:0,cross:"MIN"}); f.appendChild(left); fill(left);
    const eyebrow=txt("GALLERY",11,"Bold","#EE661D"); eyebrow.letterSpacing=1.2; left.appendChild(eyebrow);
    const h=txt("Work that\nSpeaks for\nItself.",stacked?28:40,"Bold","#111827",{lineHeight:stacked?36:48}); left.appendChild(h); fill(h);
    const sd=txt("Hand-picked projects that push\nthe boundaries of design and craft.",15,"Regular","#6B7280",{lineHeight:24}); left.appendChild(sd); fill(sd);
    left.appendChild(btnNode("View All Work","#EE661D","#FFFFFF",20,11,8));
    const rightGrid=alFrame({name:"Image Grid",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0}); f.appendChild(rightGrid); fill(rightGrid);
    [[{bg:"#F97316"},{bg:"#8B5CF6"}],[{bg:"#0EA5E9"},{bg:"#10B981"}]].forEach((row,ri)=>{
      const r=alFrame({name:"Row "+(ri+1),dir:"HORIZONTAL",gap:16,pl:0,pr:0,pt:0,pb:0}); rightGrid.appendChild(r); fill(r);
      row.forEach(({bg})=>{
        const imgF=alFrame({name:"Image",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); r.appendChild(imgF); fill(imgF);
        imgF.resize(1,stacked?120:160); imgF.primaryAxisSizingMode="FIXED";
      });
    });
    return f;
  },
  "gallery-4": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Hero — Feature Mosaic",dir:"VERTICAL",bg:"#FAFAFA",w:W,pt:60,pb:60,pl:40,pr:40,gap:40,cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const hdr=alFrame({name:"Header",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(hdr); fill(hdr);
    ...
    hdr.appendChild(btnNode("Browse Gallery","#EE661D","#FFFFFF",22,12,8));
    ...
    const mainImg=alFrame({name:"Main Image",dir:"VERTICAL",bg:"#0EA5E9",gap:0,pl:0,pr:0,pt:0,pb:0,radius:16}); mosaic.appendChild(mainImg); fill(mainImg);
    if(mobile){mainImg.resize(1,200); mainImg.primaryAxisSizingMode="FIXED";} else {fillV(mainImg);}
    ...
    [{bg:"#F97316"},{bg:"#8B5CF6"}].forEach(({bg})=>{
      const imgF=alFrame({name:"Image",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); sideStack.appendChild(imgF); fill(imgF);
      imgF.resize(1,mobile?160:140); imgF.primaryAxisSizingMode="FIXED";
    });
    return f;
  },
  "gallery-5": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Hero — Panoramic Stage",dir:"VERTICAL",bg:"#FFFFFF",w:W,pt:60,pb:60,pl:40,pr:40,gap:32,cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const hdr=alFrame({name:"Header",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(hdr); fill(hdr);
    ...
    hdr.appendChild(btnNode("See All Projects","#EE661D","#FFFFFF",22,12,8));
    const hero=alFrame({name:"Hero Image",dir:"VERTICAL",bg:"#64748B",gap:0,pl:0,pr:0,pt:0,pb:0,radius:20}); f.appendChild(hero); fill(hero);
    hero.resize(1,mobile?160:280); hero.primaryAxisSizingMode="FIXED";
    ...
    [{bg:"#F97316"},{bg:"#0EA5E9"},{bg:"#10B981"}].forEach(({bg})=>{
      const imgF=alFrame({name:"Thumb",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); strip.appendChild(imgF); fill(imgF);
      imgF.resize(1,mobile?80:100); imgF.primaryAxisSizingMode="FIXED";
    });
    return f;
  },
  "gallery-6": W => {
    const mobile=W<=480;
    const f=alFrame({name:"Hero — Studio Wall",dir:"VERTICAL",bg:"#111827",w:W,pt:60,pb:60,pl:40,pr:40,gap:40,cross:"CENTER"});
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const hdr=alFrame({name:"Header",dir:"VERTICAL",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"}); f.appendChild(hdr); fill(hdr);
    ...
    hdr.appendChild(btnNode("Start a Project","#EE661D","#FFFFFF",22,12,8));
    if(mobile){
      [{bg:"#8B5CF6"},{bg:"#F97316"},{bg:"#0EA5E9"}].forEach(({bg})=>{
        const imgF=alFrame({name:"Image",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); f.appendChild(imgF); fill(imgF);
        imgF.resize(1,160); imgF.primaryAxisSizingMode="FIXED";
      });
    } else {
      const row1=alFrame({name:"Row 1",dir:"HORIZONTAL",gap:16,pl:0,pr:0,pt:0,pb:0}); f.appendChild(row1); fill(row1);
      [{bg:"#8B5CF6"},{bg:"#F97316"},{bg:"#0EA5E9"}].forEach(({bg})=>{
        const imgF=alFrame({name:"Image",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); row1.appendChild(imgF); fill(imgF);
        imgF.resize(1,180); imgF.primaryAxisSizingMode="FIXED";
      });
      const row2=alFrame({name:"Row 2",dir:"HORIZONTAL",gap:16,pl:0,pr:0,pt:0,pb:0}); f.appendChild(row2); fill(row2);
      [{bg:"#10B981"},{bg:"#64748B"}].forEach(({bg})=>{
        const imgF=alFrame({name:"Image",dir:"VERTICAL",bg,gap:0,pl:0,pr:0,pt:0,pb:0,radius:12}); row2.appendChild(imgF); fill(imgF);
        imgF.resize(1,140); imgF.primaryAxisSizingMode="FIXED";
      });
    }
    return f;
  },
  ...
  "video-1": W => {
    ...
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const mobile=W<=480;
    f.appendChild(pillNode("New release","#1C1C1E","#FFFFFF"));
    ...
    const vf=alFrame({name:"Video Frame",dir:"VERTICAL",bg:"#1C1C1E",main:"CENTER",cross:"CENTER",radius:12,gap:12,pl:0,pr:0,pt:0,pb:0});
    ...
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"#FFFFFF",main:"CENTER",cross:"CENTER",pl:16,pr:16,pt:14,pb:14,radius:99,gap:0}); pb.appendChild(txt("▶",20,"Regular","#09090B")); vf.appendChild(pb);
    vf.appendChild(txt("Watch the demo · 2 min",13,"Regular","#71717A"));
    return f;
  },
  "video-2": W => {
    const mobile=W<=480;
    ...
    const sd=txt("Watch how teams use our platform to ship 10x faster.",15,"Regular","#6B7280",{lineHeight:24}); left.appendChild(sd); fill(sd); if(mobile) sd.textAlignHorizontal="CENTER";
    left.appendChild(btnNode("Get started free","#EE661D","#FFFFFF",20,11,8));
    ...
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"#EE661D",main:"CENTER",cross:"CENTER",pl:14,pr:14,pt:12,pb:12,radius:99,gap:0}); pb.appendChild(txt("▶",18,"Regular","#FFFFFF")); vf.appendChild(pb);
  ...
  "video-3": W => {
    ...
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"transparent",main:"CENTER",cross:"CENTER",pl:20,pr:20,pt:14,pb:14,radius:99,gap:8});
  ...
  "video-4": W => {
    ...
    const sd=txt("A quick walkthrough of the most loved features.",15,"Regular","#6B7280"); f.appendChild(sd); if(mobile){fill(sd); sd.textAlignHorizontal="CENTER";}
    ...
    f.appendChild(vf);
    vf.resize(1,Math.round((W-80)*.5625)); vf.primaryAxisSizingMode="FIXED"; fill(vf);
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"#111827",main:"CENTER",cross:"CENTER",pl:18,pr:18,pt:14,pb:14,radius:99,gap:8});
    pb.appendChild(txt("▶ Play video",13,"SemiBold","#FFFFFF")); vf.appendChild(pb);
    return f;
  },
  "video-5": W => {
    ...
    hdr.appendChild(btnNode("Start free trial","#EE661D","#FFFFFF",20,11,8));
    ...
    f.appendChild(vf);
    vf.resize(1,Math.round((W-80)*.5625)); vf.primaryAxisSizingMode="FIXED"; fill(vf);
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"#EE661D",main:"CENTER",cross:"CENTER",pl:16,pr:16,pt:12,pb:12,radius:99,gap:8});
    pb.appendChild(txt("▶",16,"Regular","#FFFFFF")); pb.appendChild(txt("Play demo",13,"SemiBold","#FFFFFF")); vf.appendChild(pb);
  ...
  "tab-h-1": W => {
    ...
    if(mobile){
      ...
      dd.appendChild(txt("Overview",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280")); bar.appendChild(dd); fill(dd);
    } else {
    ...
    content.appendChild(txt("Summary of your key metrics and activity.",13,"Regular","#6B7280"));
  ...
  "tab-h-2": W => {
    ...
    if(mobile){
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Analytics",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280")); bar.appendChild(dd); fill(dd);
    } else {
      ["Overview","Analytics","Reports","Settings"].forEach((lbl,i)=>{
        const active=i===1;
    ...
    content.appendChild(txt("Track performance across all channels.",13,"Regular","#6B7280"));
  ...
  "tab-h-3": W => {
    ...
    if(mobile){
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Products",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280")); bar.appendChild(dd); fill(dd);
    } else {
    ...
    ["Headphones Pro — $89","Wireless Mouse — $34","USB-C Hub — $45"].forEach((item,i)=>{
      ...
      row.appendChild(txt(item,13,"Regular","#374151")); row.appendChild(txt("In Stock",11,"Medium","#10B981")); tbl.appendChild(row); fill(row);
    });
  ...
  "tab-h-4": W => {
    ...
    if(mobile){
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Quarterly",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280")); bar.appendChild(dd); fill(dd);
    } else {
    ...
    content.appendChild(txt("Q3 2024 performance summary across all regions.",13,"Regular","#6B7280"));
  ...
  "tab-h-5": W => {
    ...
    if(mobile){
      ...
      dd.appendChild(txt("Dashboard",13,"SemiBold","#F1F5F9")); dd.appendChild(txt("▾",13,"Regular","#64748B")); bar.appendChild(dd); fill(dd);
    } else {
    ...
    content.appendChild(txt("Real-time activity and system overview.",13,"Regular","#64748B"));
  ...
  "tab-h-6": W => {
    ...
    if(mobile){
      ...
      dd.appendChild(txt("All",13,"SemiBold","#7C3AED")); dd.appendChild(txt("▾",13,"Regular","#6D28D9")); bar.appendChild(dd); fill(dd);
    } else {
      ["All","Published","Drafts","Archived"].forEach((lbl,i)=>{
        const active=i===0;
        const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#7C3AED":"transparent",pl:16,pr:16,pt:8,pb:8,main:"CENTER",cross:"CENTER",radius:99});
    ...
    ["Launch blog post","Product update draft","Q4 roadmap","Team announcement"].forEach((item,i)=>{
      ...
      row.appendChild(txt(item,13,"Regular","#374151"));
      const badge=alFrame({name:"Badge",dir:"HORIZONTAL",bg:i<2?"#D1FAE5":"#F3F4F6",pl:8,pr:8,pt:3,pb:3,radius:99,main:"CENTER",cross:"CENTER"});
  ...
  "tab-v-1": W => {
    ...
    if(sm){
      ...
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Overview",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
      const tabs=alFrame({name:"Tabs",dir:"VERTICAL",bg:"#F9FAFB",gap:4,pl:8,pr:8,pt:8,pb:8,radius:10});
      ...
      ["Overview","Analytics","Settings","Billing","Support"].forEach((lbl,i)=>{
        const active=i===0;
        const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#FFFFFF":"transparent",pl:12,pr:12,pt:10,pb:10,gap:10,cross:"CENTER",radius:10});
        if(active){tab.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; tab.strokeWeight=1;}
    ...
    const content=alFrame({name:"Content",dir:"VERTICAL",bg:"#FFFFFF",pl:40,pr:40,pt:40,pb:40,gap:24,radius:10});
    content.appendChild(txt("Overview",20,"Bold","#111827"));
    content.appendChild(txt("Manage your account details and preferences.",13,"Regular","#6B7280"));
    ["Name","Email","Company","Role"].forEach(lbl=>{
      ...
      field.appendChild(txt(lbl,11,"SemiBold","#374151"));
      const input=alFrame({name:"Input",dir:"HORIZONTAL",bg:"#F9FAFB",pl:12,pr:12,pt:10,pb:10,radius:10,cross:"CENTER"});
      input.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; input.strokeWeight=1;
      input.appendChild(txt(lbl+" value",13,"Regular","#9CA3AF")); field.appendChild(input); fill(input); content.appendChild(field); fill(field);
    });
    f.appendChild(content); fill(content); return f;
  },
  "tab-v-2": W => {
    ...
    if(sm){
      ...
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Dashboard",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
      ...
      ["Dashboard","Users","Campaigns","Reports","Settings"].forEach((lbl,i)=>{
        const active=i===0;
        const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#EEF2FF":"transparent",pl:12,pr:12,pt:12,pb:12,gap:12,cross:"CENTER",radius:10});
    ...
    content.appendChild(txt("Welcome back. Here's what's happening today.",13,"Regular","#6B7280"));
    ...
    [["2,840","Active users"],["94%","Uptime"],["$18.4k","MRR"]].forEach(([v,l])=>{
      const c=alFrame({name:l,dir:"VERTICAL",bg:"#F9FAFB",pl:16,pr:16,pt:16,pb:16,gap:4,radius:10}); fill(c);
      c.appendChild(txt(v,22,"Bold","#111827")); c.appendChild(txt(l,11,"Regular","#9CA3AF")); grid.appendChild(c); fill(c);
    });
  ...
  "tab-v-3": W => {
    ...
    if(sm){
      ...
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#1E293B",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#334155")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Deployments",13,"SemiBold","#F1F5F9")); dd.appendChild(txt("▾",13,"Regular","#64748B"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
      const tabs=alFrame({name:"Tabs",dir:"VERTICAL",bg:"#1E293B",gap:2,pl:8,pr:8,pt:8,pb:8,radius:10});
      tabs.primaryAxisSizingMode="AUTO";
      ["Overview","Deployments","Logs","Metrics","Settings"].forEach((lbl,i)=>{
        const active=i===1;
        const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#334155":"transparent",pl:12,pr:12,pt:10,pb:10,gap:10,cross:"CENTER",radius:10});
        tab.appendChild(txt(lbl,13,active?"SemiBold":"Regular",active?"#F1F5F9":"#64748B")); tabs.appendChild(tab); fill(tab);
      });
      f.appendChild(tabs); fill(tabs);
    }
    const content=alFrame({name:"Content",dir:"VERTICAL",bg:"#1E293B",pl:40,pr:40,pt:40,pb:40,gap:24,radius:10});
    content.appendChild(txt("Deployments",20,"Bold","#F1F5F9"));
    content.appendChild(txt("Active deployments across all environments.",13,"Regular","#64748B"));
    ["prod-v2.4.1 · Live","staging-v2.5.0 · Building","dev-main · Ready"].forEach((item,i)=>{
      const row=alFrame({name:"Deploy",dir:"HORIZONTAL",bg:"#0F172A",pl:16,pr:16,pt:14,pb:14,main:"SPACE_BETWEEN",cross:"CENTER",radius:10}); fill(row);
      row.appendChild(txt(item,12,"Regular","#94A3B8"));
      const dot=alFrame({name:"Status",dir:"HORIZONTAL",bg:i===0?"#D1FAE5":i===1?"#FEF3C7":"#E0F2FE",pl:8,pr:8,pt:3,pb:3,radius:99,main:"CENTER",cross:"CENTER"});
  ...
  "tab-v-4": W => {
    ...
    if(sm){
      ...
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Notifications",13,"SemiBold","#111827")); dd.appendChild(txt("▾",13,"Regular","#6B7280"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
      const tabs=alFrame({name:"Tabs",dir:"VERTICAL",bg:"#F3F4F6",gap:2,pl:6,pr:6,pt:6,pb:6,radius:10});
      tabs.primaryAxisSizingMode="AUTO";
      ["General","Security","Notifications","Integrations","Advanced"].forEach((lbl,i)=>{
        const active=i===2;
        const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#FFFFFF":"transparent",pl:14,pr:14,pt:9,pb:9,cross:"CENTER",radius:99});
        if(active){tab.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; tab.strokeWeight=1;}
        tab.appendChild(txt(lbl,13,active?"SemiBold":"Regular",active?"#111827":"#6B7280")); tabs.appendChild(tab); fill(tab);
      });
      f.appendChild(tabs); fill(tabs);
    ...
    [["Email alerts","Get notified via email"],["Push notifications","In-app alerts"],["Weekly digest","Summary every Monday"],["Marketing emails","Updates and offers"]].forEach(([title,desc],i)=>{
      ...
      info.appendChild(txt(title,13,"SemiBold","#111827")); info.appendChild(txt(desc,12,"Regular","#9CA3AF")); row.appendChild(info);
      const tog=alFrame({name:"Toggle",dir:"HORIZONTAL",bg:i<2?"#6366F1":"#E5E7EB",pl:2,pr:2,pt:2,pb:2,radius:99,cross:"CENTER",main:i<2?"MAX":"MIN"});
      tog.resize(36,20); tog.primaryAxisSizingMode="FIXED"; tog.counterAxisSizingMode="FIXED";
      const knob=alFrame({name:"Knob",dir:"HORIZONTAL",bg:"#FFFFFF",pl:0,pr:0,pt:0,pb:0,radius:99}); knob.resize(16,16); knob.primaryAxisSizingMode="FIXED"; knob.counterAxisSizingMode="FIXED";
  ...
  "tab-v-5": W => {
    ...
    if(sm){
      ...
      const dd=alFrame({name:"Tab Dropdown",dir:"HORIZONTAL",bg:"#FFFFFF",pl:12,pr:12,pt:10,pb:10,gap:8,radius:8,main:"SPACE_BETWEEN",cross:"CENTER"});
      dd.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; dd.strokeWeight=1;
      dd.appendChild(txt("Components",13,"SemiBold","#15803D")); dd.appendChild(txt("▾",13,"Regular","#6B7280"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
    ...
    content.appendChild(txt("A collection of ready-to-use UI components.",13,"Regular","#6B7280"));
    ...
    ["Button","Input","Card","Badge","Modal","Avatar"].forEach(comp=>{
      const c=alFrame({name:comp,dir:"HORIZONTAL",bg:"#F9FAFB",pl:12,pr:12,pt:8,pb:8,cross:"CENTER",radius:10,gap:8});
      c.strokes=[{type:"SOLID",color:rgb("#E5E7EB")}]; c.strokeWeight=1;
  ...
  "tab-v-6": W => {
    ...
    if(sm){
      ...
      dd.appendChild(txt("Profile",13,"SemiBold","#FFFFFF")); dd.appendChild(txt("▾",13,"Regular","#9CA3AF"));
      f.appendChild(bar); fill(bar); bar.appendChild(dd); fill(dd);
    } else {
      ...
      sections.forEach(([heading,items])=>{
        ...
        items.forEach((lbl,i)=>{
          ...
          const tab=alFrame({name:"Tab: "+lbl,dir:"HORIZONTAL",bg:active?"#1F2937":"transparent",pl:12,pr:12,pt:9,pb:9,cross:"CENTER",radius:10});
    ...
    const content=alFrame({name:"Content",dir:"VERTICAL",bg:"#1F2937",pl:40,pr:40,pt:40,pb:40,gap:24,radius:10});
    content.appendChild(txt("Profile",20,"Bold","#FFFFFF"));
    content.appendChild(txt("Manage your public profile information.",13,"Regular","#9CA3AF"));
    const avatar=alFrame({name:"Avatar Row",dir:"HORIZONTAL",bg:"#1F2937",gap:16,pl:0,pr:0,pt:0,pb:0,cross:"CENTER"});
    const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#6366F1",pl:0,pr:0,pt:0,pb:0,main:"CENTER",cross:"CENTER",radius:99}); av.resize(56,56); av.primaryAxisSizingMode="FIXED"; av.counterAxisSizingMode="FIXED";
    ...
    ["Full name","Username","Bio"].forEach(lbl=>{
      ...
      field.appendChild(txt(lbl,11,"SemiBold","#9CA3AF"));
      const input=alFrame({name:"Input",dir:"HORIZONTAL",bg:"#111827",pl:12,pr:12,pt:10,pb:10,radius:10,cross:"CENTER"});
      input.strokes=[{type:"SOLID",color:rgb("#374151")}]; input.strokeWeight=1;
      input.appendChild(txt("Enter "+lbl.toLowerCase(),13,"Regular","#4B5563")); field.appendChild(input); fill(input); content.appendChild(field); fill(field);
    });
    f.appendChild(content); fill(content); return f;
  },
  ...
  "vnav-1": W => {
    ...
    const badge=rect(28,28,"#6366F1",8); logo.appendChild(badge); logo.appendChild(txt("Brand",15,"Bold","#111827")); f.appendChild(logo); fill(logo);
  ...
  "vnav-2": W => {
    ...
    const badge=rect(28,28,"#6366F1",8); logo.appendChild(badge); logo.appendChild(txt("Brand",15,"Bold","#FFFFFF")); f.appendChild(logo); fill(logo);
    ...
    const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#6366F1",pl:0,pr:0,pt:0,pb:0,main:"CENTER",cross:"CENTER",radius:99}); av.resize(28,28); av.primaryAxisSizingMode="FIXED"; av.counterAxisSizingMode="FIXED";
  ...
  "vnav-4": W => {
    ...
    search.appendChild(txt("🔍",11,"Regular","#9CA3AF")); search.appendChild(txt("Search...",12,"Regular","#9CA3AF")); f.appendChild(search); fill(search);
    ...
    [["Inbox","3"],["Today",null],["Projects",null],["Archive",null]].forEach(([lbl,badge],i)=>{
      ...
      if(badge){const b=alFrame({name:"Badge",dir:"HORIZONTAL",bg:"#6366F1",pl:7,pr:7,pt:2,pb:2,radius:99,main:"CENTER",cross:"CENTER"}); b.appendChild(txt(badge,10,"Bold","#FFFFFF")); item.appendChild(b);}
  ...
  "vnav-5": W => {
    ...
    f.primaryAxisSizingMode="AUTO"; f.counterAxisSizingMode="FIXED";
    const logo=alFrame({name:"Logo",dir:"HORIZONTAL",bg:"#6366F1",pl:0,pr:0,pt:0,pb:0,main:"CENTER",cross:"CENTER",radius:10}); logo.resize(40,40); logo.primaryAxisSizingMode="FIXED"; logo.counterAxisSizingMode="FIXED";
    ...
    const av=alFrame({name:"Avatar",dir:"HORIZONTAL",bg:"#6366F1",pl:0,pr:0,pt:0,pb:0,main:"CENTER",cross:"CENTER",radius:99}); av.resize(40,40); av.primaryAxisSizingMode="FIXED"; av.counterAxisSizingMode="FIXED";
  ...
  "vnav-6": W => {
    ...
    header.appendChild(txt("Navigation",13,"SemiBold","#111827"));
    ...
    [["Home",true],["Products",false],["Pricing",false],["About",false],["Blog",false],["Contact",false]].forEach(([lbl,active])=>{
      ...
      item.appendChild(txt(lbl,13,active?"SemiBold":"Regular",active?"#6366F1":"#374151"));
      if(active) item.appendChild(txt("→",13,"Regular","#6366F1"));
      nav.appendChild(item); fill(item);
  ...
  "video-6": W => {
    ...
    f.appendChild(vf);
    vf.resize(1,Math.round((W-80)*.5625)); vf.primaryAxisSizingMode="FIXED"; fill(vf);
    const pb=alFrame({name:"Play Button",dir:"HORIZONTAL",bg:"#FFFFFF",main:"CENTER",cross:"CENTER",pl:20,pr:20,pt:20,pb:20,radius:99,gap:0});
...
function buildGeneric(sectionId, variantStyle, sectionName, W) {
  ...
  f.appendChild(txt("A unique variation of the "+sectionName.toLowerCase()+" section.",15,"Regular",p.sc));
